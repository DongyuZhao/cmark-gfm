...
static void commonmark_render(cmark_syntax_extension *extension,
  ...
  const char *literal = cmark_gfm_extensions_get_formula_literal(node);
  if (!literal) {
    return;
  }
...
static void html_render(cmark_syntax_extension *extension,
  ...
  cmark_strbuf_put(renderer->html, literal, node->as.literal.len);
...
const char *cmark_gfm_extensions_get_formula_literal(cmark_node *node) {
  ...
  return (const char *)cmark_chunk_to_cstr(cmark_node_mem(node), &node->as.literal);
}
